import { SHORT_CHARACTERS, LONG_CHARACTERS } from "@/constants/string.constant";
...
export const addOpacityToColorHelper = (color: string, opacity: number) => {
  // Check if the color is in HEX format using compiled regex
  ...
  // Check if the color is in RGB or RGBA format using compiled regex
  else if (RGB_RGBA_REGEX.test(color)) {
    // Extract the RGB(A) values more efficiently
    const start = color.indexOf("(") + 1;
    const end = color.lastIndexOf(")");
    const values = color.slice(start, end).split(",");
...
export function replaceSvgColorsHelper(
  ...
  COLOR_ATTRIBUTE_REGEX.lastIndex = 0;

  // Replace all matched color attributes with the new color using compiled regex
  const updatedSvg = svgString.replace(COLOR_ATTRIBUTE_REGEX, (match, attr) => {
